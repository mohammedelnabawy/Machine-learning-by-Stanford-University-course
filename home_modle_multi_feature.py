import numpy as np
import pandas as pd
import matplotlib.pyplot as plt


#read data from the source
path = 'home_price.txt'
data2 = pd.read_csv(path, header=None, names=['Size', 'Bedrooms', 'Price'])

#show data 
print('data = ')
print(data2.head(10) )


# rescaling data
'''we rescale data to meny reason, to increase the trainning process and 
Don't be skewed in one direction becouse of one of the feature have a very large range than other feature
make rescale to make the range of the feature are close to each other'''
data2 = (data2 - data2.mean()) / data2.std()

#show data after rescaling
print('data after normalization = ')
print(data2.head(10) )


# adding a new column called ones to the data have value (1) (x0)
data2.insert(0, 'Ones', 1)


# separate X (training data) from y (target variable)
cols = data2.shape[1]
X2 = data2.iloc[:,0:cols-1]
y2 = data2.iloc[:,cols-1:cols]


print('**************************************')
print('X2 data = \n' ,X2.head(10) ) #print the 'X' values
print('y2 data = \n' ,y2.head(10) ) #print the 'Y' values
print('**************************************')


# convert from data frames to numpy matrices
X2 = np.matrix(X2.values)
y2 = np.matrix(y2.values)
theta2 = np.matrix(np.array([0,0,0])) # number of fields in the array must be equal number of feature + 1


# cost functhion = (1/2m)∑(h(x) - y)^2
def computeCost(X, y, theta):
    inner = np.power(((X * theta.T) - y), 2)
    return np.sum(inner) / (2 * len(X))


# GD function
# θj = θj -(1/m)∑(h(x) - y)
def gradientDescent(X, y, theta, alpha, iters):
    temp = np.matrix(np.zeros(theta.shape))
    parameters = int(theta.ravel().shape[1])
    cost = np.zeros(iters)
    
    for i in range(iters):
        error = (X * theta.T) - y
        
        for j in range(parameters):
            term = np.multiply(error, X[:,j])
            temp[0,j] = theta[0,j] - ((alpha / len(X)) * np.sum(term))
            
        theta = temp
        cost[i] = computeCost(X, y, theta)
        
    return theta, cost


# initialize variables for learning rate and iterations
alpha = 0.1 # learning rate 
iters = 100 # number of iteration

# # perform gradient descent to "fit" the model parameters
g2, cost2 = gradientDescent(X2, y2, theta2, alpha, iters)

# get the cost (error) of the model
thiscost = computeCost(X2, y2, g2)


print('g2 = ' , g2)
print('cost2  = ' , cost2[0:50] )
print('computeCost = ' , thiscost)
print('**************************************')


# get best fit line for Size vs. Price

x = np.linspace(data2.Size.min(), data2.Size.max(), 100)
# print('x \n',x)
# print('g \n',g2)
f = g2[0, 0] + (g2[0, 1] * x)
# print('f \n',f)


# draw the line for Size vs. Price
fig, ax = plt.subplots(figsize=(5,5))
ax.plot(x, f, 'r', label='Prediction')
ax.scatter(data2.Size, data2.Price, label='Training Data')
ax.legend(loc=2)
ax.set_xlabel('Size')
ax.set_ylabel('Price')
ax.set_title('Size vs. Price')


# get best fit line for Bedrooms vs. Price
x = np.linspace(data2.Bedrooms.min(), data2.Bedrooms.max(), 100)
# print('x \n',x)
# print('g \n',g2)
f = g2[0, 0] + (g2[0, 1] * x)
# print('f \n',f)
# draw the line  for Bedrooms vs. Price
fig, ax = plt.subplots(figsize=(5,5))
ax.plot(x, f, 'r', label='Prediction')
ax.scatter(data2.Bedrooms, data2.Price, label='Traning Data')
ax.legend(loc=2)
ax.set_xlabel('Bedrooms')
ax.set_ylabel('Price')
ax.set_title('Size vs. Price')


# draw error graph
fig, ax = plt.subplots(figsize=(5,5))
ax.plot(np.arange(iters), cost2, 'r')
ax.set_xlabel('Iterations')
ax.set_ylabel('Cost')
ax.set_title('Error vs. Training Epoch')
